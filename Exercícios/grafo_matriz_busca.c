...
void visitaDFS(int u, TipoGrafo *grafo, int *tempo, int *d, int *t, int
*cor, int *ant);
...
*cor, int *ant){
	...
	(*tempo)++;
	d[u] = (*tempo);
	printf("Visita %2d tempo descoberta: %2d cinza %d \n",u,d[u],cor[u]);
...
void buscaEmProfundidade(TipoGrafo *grafo){
	...
	int cor[MaxNumVertices+1]; /* cores: 0 - branco, 1 - cinza, 2 - preto*/
