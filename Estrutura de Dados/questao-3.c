...
void imprimeListaInvert(TipoLista lista);
/* operacoes do grafo */
...
*cor, int *ant, TipoLista *listaTopo);
...
int main(){
	...
	insereAresta(0,5,1,grafo);
	insereAresta(1,2,1,grafo);
	insereAresta(2,3,1,grafo);
	insereAresta(2,4,1,grafo);
	insereAresta(4,6,1,grafo);
	insereAresta(5,4,1,grafo);
	insereAresta(5,6,1,grafo);
	insereAresta(6,7,1,grafo);
	insereAresta(6,8,1,grafo);
	insereAresta(7,8,1,grafo);
	insereAresta(9,6,1,grafo);
...
*cor, int *ant, TipoLista *listaTopo){
	...
	if(!listaAdjVazia(u,grafo)){
		...
		while (!fimListaAdj){
			...
			if(cor[v]=='b'){				
				ant[v] = u;
				visitaDFS(v,grafo,tempo,d,t,cor,ant, listaTopo);
			}
		}
	}
	cor[u] = 'p';
...
void buscaEmProfundidade(TipoGrafo *grafo){
	...
	for(x = 0; x<grafo->numVertices;x++){
		if(cor[x]=='b'){
			visitaDFS(x, grafo,&tempo,d,t, cor, ant, &listaTopo);
		}	
	}

	printf("\n\n\n");
	printf("Ordenacao Topologica:\n");
	imprimeListaInvert(listaTopo);
}
